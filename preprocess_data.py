...
def val(config):
    ...
    print("create PreprocessDataset")
...
def modify(config, data_loader, save):
    ...
    for i, data in enumerate(data_loader): # data is graph
...
def preprocess(graph, cross_dist, cross_angle=None):
    ...
    dist = graph['ctrs'].unsqueeze(1) - graph['ctrs'].unsqueeze(0) # [Ns,2] [Ns,2] => [Ns,1,2] [1,Ns,2] => [Ns,Ns,2]
    dist = torch.sqrt((dist ** 2).sum(2)) # [Ns,Ns]
    hi = torch.arange(num_nodes).long().to(dist.device).view(-1, 1).repeat(1, num_nodes).view(-1) # [Ns,1]=>[Ns,Ns]=>[Ns*Ns]
    wi = torch.arange(num_nodes).long().to(dist.device).view(1, -1).repeat(num_nodes, 1).view(-1) # [1,Ns]=>[Ns,Ns]=>[Ns*Ns]
    row_idcs = torch.arange(num_nodes).long().to(dist.device) # [Ns]
    ...
    if len(pairs) > 0:
        ...
        mask = mat[lane_idcs[hi], lane_idcs[wi]].logical_not() # [Ns*Ns]
        left_dist[hi[mask], wi[mask]] = 1e6 # seg2seg relation is not left, left-pre, or left-suc, its left_dist is +inf
        if cross_angle is not None:
            left_dist[hi[left_mask], wi[left_mask]] = 1e6

        min_dist, min_idcs = left_dist.min(1) # [Ns]
