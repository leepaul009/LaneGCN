...
class Net(nn.Module):
    ...
    def forward(self, data: Dict) -> Dict[str, List[Tensor]]:
        ...
        actors = self.actor_net(actors) # =>[N,128]
...
def actor_gather(actors: List[Tensor]) -> Tuple[Tensor, List[Tensor]]:
    ...
    actors = [x.transpose(1, 2) for x in actors] # list[ [N_agents, 3, 20] ]
    actors = torch.cat(actors, 0) # [N_agents_in_batch, 3, 20]
...
class MapNet(nn.Module):
    ...
    def forward(self, graph):
        ...
        feat = self.input(ctrs) # [Ns,2]=>[Ns,128]
...
class PredNet(nn.Module):
    ...
    def __init__(self, config):
        ...
        pred = [] 
        for i in range(config["num_mods"]): # 6 number of mode
            pred.append(
                nn.Sequential(
                    LinearRes(n_actor, n_actor, norm=norm, ng=ng),
                    nn.Linear(n_actor, 2 * config["num_preds"]), # 128 => 2*30
                )
            )
    ...
    def forward(self, actors: Tensor, # [N_a, 128, 1]
        ...
        for i in range(len(self.pred)): # 6
            preds.append(self.pred[i](actors)) # [N_a,60,1]
        reg = torch.cat([x.unsqueeze(1) for x in preds], 1)
        reg = reg.view(reg.size(0), reg.size(1), -1, 2) # [N,6,30,2]
        ...
        feats = self.att_dest(actors, torch.cat(actor_ctrs, 0), dest_ctrs) # [N*6,128]
...
class Att(nn.Module):
    ...
    """
    def __init__(self, n_agt: int, n_ctx: int) -> None: # ex. n_mnode:128, n_agent:128
    ...
    def forward(self, 
                agts: Tensor, # map feat [Ns, 128+4]
                agt_idcs: List[Tensor], # graph["idcs"], node id in global(batch)
                agt_ctrs: List[Tensor], # graph["ctrs"]
                ctx: Tensor, # actor feat
                ctx_idcs: List[Tensor], # agent id in global(batch)
                ctx_ctrs: List[Tensor], #
                dist_th: float) -> Tensor: # 7.0
        res = agts
        if len(ctx) == 0:
            agts = self.agt(agts)
            agts = self.relu(agts)
        ...
        # get map node id and agent id that have distance-association
        hi = torch.cat(hi, 0) # cat along batch
        wi = torch.cat(wi, 0)

        agt_ctrs = torch.cat(agt_ctrs, 0)
        ctx_ctrs = torch.cat(ctx_ctrs, 0)
        dist = agt_ctrs[hi] - ctx_ctrs[wi] # use hi/wi get related node and calculate dist
        dist = self.dist(dist)

        query = self.query(agts[hi]) # dist-assoc map node feat as input (fc:map_n->agt_n)

        ctx = ctx[wi] # dist-assc agent feat
        ctx = torch.cat((dist, query, ctx), 1)
        ctx = self.ctx(ctx)

        agts = self.agt(agts)
        agts.index_add_(0, hi, ctx) # add learned feat into map feat
        agts = self.norm(agts)
        agts = self.relu(agts)
...
class AttDest(nn.Module):
    ...
    def forward(self, agts: Tensor, agt_ctrs: Tensor, dest_ctrs: Tensor) -> Tensor:
        n_agt = agts.size(1) # 128
        num_mods = dest_ctrs.size(1) # 6

        dist = (agt_ctrs.unsqueeze(1) - dest_ctrs).view(-1, 2) # [N,1,2]-[N,6,2]=>[N,6,2]=>[N*6,2]
        dist = self.dist(dist) # [N*6,2]=>[N*6,128]
        agts = agts.unsqueeze(1).repeat(1, num_mods, 1).view(-1, n_agt) #[N,1,128]=>[N,6,128]=>[N*6,128]

        agts = torch.cat((dist, agts), 1) # [N*6,256]
        agts = self.agt(agts) # [N*6,128]
        return agts
